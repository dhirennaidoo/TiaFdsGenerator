...
namespace TiaFds.Extract.Cli
{
    internal static class Program
    {
        ...
        {
            TiaProjectResult result = new TiaProjectReader().Read(options.Input, options.RetrieveTo, options.PlcName);
        ...
        private sealed class ExtractOptions
        {
            ...
            public bool Verbose, Inventory, Overwrite, IncludeSourcePath;
            ...
            public static ExtractOptions Parse(string[] args)
            {
                ...
                if ((result.Inventory || result.ExportJson != null) && string.IsNullOrWhiteSpace(result.PlcName)) throw Invalid("--inventory and --export-json require --plc <name>.");
            ...
            private static string Usage() { return "Usage: TiaFds.Extract.Cli.exe --input <path> [--retrieve-to <folder>] [--plc <name>] [--inventory] [--verbose] [--export-json <path>] [--overwrite] [--include-source-path]"; }
        }
    }
}
